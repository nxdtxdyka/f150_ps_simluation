...
!Modification Tags: Generate random numbers following an exponential distribution.
function RandomExponential(lambda) result(random_value)
  implicit none
  real(8), intent(in) :: lambda  ! 指数分布参数
  real(8) :: random_value

  !Generate random numbers following a uniform distribution.
  call random_number(random_value)

  ! Transform uniform random numbers into exponential random numbers.
  random_value = -log(1.0 - random_value) / lambda
end function RandomExponential




!Modification Tags:Generate Gaussian distributed random numbers with specified mean and variance.
REAL FUNCTION random_gaussian(mean, variance)
  implicit none
  REAL, INTENT(IN) :: mean  ! 期望值（均值）
  REAL, INTENT(IN) :: variance  ! 方差

  REAL :: u1, u2, z, x
  REAL, PARAMETER :: two_pi = 2.0 * 3.14159265358979323846
...
!Modification Tags:The ellipticity model for SFG is sourced from (Tunbridge et al., 2016)
  function generate_random_ell() result(random_x)
    double precision, parameter :: PI = 3.14159265358979323846
    double precision, parameter :: A = 0.19
    double precision, parameter :: NORMALIZATION_CONSTANT = 0.017840579308397237
    double precision, parameter :: MAX_P_X = 2.1853190433335827
    double precision :: random_x, u, q, px_val


    do
      call random_number(random_x) ! Generate uniform random numbers x within the interval [0,1].
      call random_number(u)        ! Generate uniform random numbers u for acceptance testing.
      px_val = p_ell(random_x, PI, A, NORMALIZATION_CONSTANT) ! Calculate the probability density function of the current x, p(x).
      q = MAX_P_X * u              ! Scale the random number u to the maximum p(x).
      if (q <= px_val) exit        ! If u is less than or equal to p(x)/max_p(x), accept x.
    end do
  end function generate_random_ell

  !Modification Tags:The ellipticity model for SFG is sourced from (Tunbridge et al., 2016) 
  function p_ell(x, PI, A, NORMALIZATION_CONSTANT) result(px)
    double precision, intent(in) :: x, PI, A, NORMALIZATION_CONSTANT
    double precision :: px
    px = x * (cos(PI * x / 2)**2) * exp(-((2 * x / A)**0.58)) / NORMALIZATION_CONSTANT
  end function p_ell
...
!of SFG and RQAGN based on Formula 2 from Best et al. (2023), assign SFR values accordingly.
  REAL FUNCTION flux_random_generator_sfg(flux)
  IMPLICIT NONE
  REAL, INTENT(IN) :: flux
  REAL, DIMENSION(18) :: data
  INTEGER :: i
  ...
  !flux mean variance
  data = ([&
    &0.00014264038732150037, 0.0301952449164195, 0.05752196462992524,&
 &0.00022606977883586258, -0.04632232364712518, 0.05907369930871991,&
 &0.00035829645349814786, -0.11781933305805693, 0.06068598642517026,&
 &0.0005678616100321747, -0.17297560717177216, 0.0623507478598694,&
 &0.0009000000000000007, -0.19829673257638863, 0.0654516058140872,&
 &1000., -0.18500901384291668, 0.06949776845346906])

  ! Generate a random number based on the input flux
  DO i = 1, SIZE(data, DIM=1)
    IF (flux <= data(i*3-2)) THEN
      generated_value = random_gaussian(data(i*3-1), data(i* 3))
      flux_random_generator_sfg = generated_value
      IF (is_nan(generated_value)) THEN
      PRINT *, "Threshold:", data(i*3-2), "Mean:", data(i*3-1), "Var:", data(i*3)
      print*, 'NaN detected!'
      print*, 'Mean:', data(i*3-1)
      print*, 'Variance:',  data(i* 3)
    END IF
      RETURN
    END IF
  END DO

  !print*,data(SIZE(data, DIM=1), 2)
  !print*,data(SIZE(data, DIM=1), 3)
  flux_random_generator_sfg = random_gaussian(data(17), data(18))
  
  END FUNCTION flux_random_generator_sfg
  
  
  
!Modification Tags:After computing the luminosity-SFR correlation for different flux intervals 
!of SFG and RQAGN based on Formula 2 from Best et al. (2023), assign SFR values accordingly.
   REAL FUNCTION flux_random_generator_rqagn(flux)
  IMPLICIT NONE
  REAL, INTENT(IN) :: flux
  REAL, DIMENSION(18) :: data
  INTEGER :: i
  ...
  !flux mean variance
  data = ([&
      &0.00014264038732150037, 0.059353854252679465, 0.06550479084457247,&
 &0.00022606977883586258, -0.04776059543935508, 0.058223665988734276,&
 &0.00035829645349814786, -0.17898651444253894, 0.06031393852878487,&
 &0.0005678616100321747, -0.24405248082089073, 0.06598175059023677,&
 &0.0009000000000000007, -0.32765729933248355, 0.0640575885848945,&
 &1000., -0.35341488742888894, 0.0782513050843805])
  
  ! Generate a random number based on the input flux
  DO i = 1, SIZE(data, DIM=1)
    IF (flux <= data(i*3-2)) THEN
      generated_value = random_gaussian(data(i*3-1),data(i*3))
      flux_random_generator_rqagn = generated_value
      IF (is_nan(generated_value)) THEN
      PRINT *, "Threshold:", data(i*3-2), "Mean:", data(i*3-1), "Var:", data(i*3)
      print*, 'NaN detected!'
      print*, 'Mean:', data(i*3-1)
  ...
  !print*,data(SIZE(data, DIM=1), 2)
  !print*,data(SIZE(data, DIM=1), 3)
  flux_random_generator_rqagn = random_gaussian(data(17), &
                                          data(18))
  END FUNCTION flux_random_generator_rqagn
